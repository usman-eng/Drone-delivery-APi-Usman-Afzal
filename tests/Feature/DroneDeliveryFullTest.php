...
class DroneDeliveryFullTest extends TestCase
{
    ...
    public function testAdminCanGetBulkOrdersAndUpdateOrderLocation()
    {
        ...
        $drone = Drone::factory()->create();
        $order = Order::factory()->pickedUp($drone)->create();

        //Test bulk orders retrieval
        $res = $this->withHeader('Authorization', $this->jwtFor($admin))
            ->getJson("/api/admin/orders");

        $res->assertStatus(200)
            ->assertJsonFragment(['id' => $order->id]);

        // Test updating order location
        $payload = [
            "origin_address" => "Main Warehouse, King Fahd Road, Riyadh",
            "origin_lat" => 24.7136,
            "origin_lng" => 46.6753,
            "destination_address" => "Customer Villa, Al Olaya, Riyadh",
            "destination_lat" => 24.7058,
            "destination_lng" => 46.6825
        ];

        $res = $this->withHeader('Authorization', $this->jwtFor($admin))
            ->putJson("/api/admin/orders/{$order->id}/location", $payload);

        $res->assertStatus(200)
            ->assertJsonFragment(['destination_address' => 'Customer Villa, Al Olaya, Riyadh']);
    }


    public function testAdminCanMarkDroneBrokenAndFixed()
    {
        $admin = User::factory()->admin()->create();
        $drone = Drone::factory()->create();
        $order = Order::factory()->reserved($drone)->create();
        ...
        // Mark broken
        $res = $this->withHeader('Authorization', $this->jwtFor($admin))
            ->postJson("/api/admin/drones/{$drone->id}/mark", ['action' => 'broken']);
        $res->assertStatus(200)->assertJsonFragment(['status' => 'broken']);
        $this->assertDatabaseHas('orders', ['id' => $order->id, 'status' => 'handoff_pending']);

        // Mark fixed
        $res = $this->withHeader('Authorization', $this->jwtFor($admin))
            ->postJson("/api/admin/drones/{$drone->id}/mark", ['action' => 'fixed']);
        $res->assertStatus(200)->assertJsonFragment(['status' => 'idle']);
    }
}
